...
while True:
    ...
    if len(lmlist)!=0:
        ...
        if fingers[1] == 1 and fingers[2] == 1 and fingers[3] == 1:   # If fore finger, middle finger, and ring finger are up
            length, img, lineInfo = detector.findDistance(8, 12, img)

            if length < 40:     # If all three fingers are really close to each other
                cv2.circle(img, (lineInfo[4], lineInfo[5]), 15, (0, 255, 0), cv2.FILLED)
                autopy.mouse.click(autopy.mouse.Button.RIGHT)    # Perform Right Click

        if all(fingers):  # If all fingers are up
            break
